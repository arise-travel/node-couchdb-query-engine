...
describe('test', () => {
    ...
    describe('other and corner cases', () => {
        ...
        it('should work with example #1', () => {
            y({
                meta: { duration: { milliseconds: 1, seconds: 0.001, readable: { seconds: '0.00 sec' } }, bg: false },
                group: { name: 'hello-a' }
            }, { 'group.name': { $eq: 'hello-a' }, 'meta.bg': { '$eq': false } });
        });

        it('should work with example #2', () => {
            n({
                'meta': {
                    'duration': { 'milliseconds': 1, 'seconds': 0.001, 'readable': { 'seconds': '0.00 sec' } },
                    'bg': false
                }
            }, { 'group.name': { '$eq': 'hello-a' }, 'meta.bg': { '$eq': 0 } });
        });
